...
class Generator {
  ...
  async getRepo() {
    ...
    const repoList = [{ id: 1, name: 'vue2 + template' }, { id: 2, name: 'vue2 + electron + template' }];
    if (!repoList) return;
  ...
  async download(repo, targetAir, params) {
    ...
    // 拼接下载地址
    let requestUrl = `https://github.com:zh317626/vue2-template#master`;
    // const requestUrl = `https://github.com:zh317626/json-tool#test`;
    if (repo == 'vue2 + electron + template') {
      requestUrl = `https://github.com:zh317626/vue2-template#master`;
    ...
    downloadGitRepo(requestUrl, path.resolve(process.cwd(), this.targetDir), (err) => {
      ...
      if (err) {
        ...
        console.log(`  npm run ${chalk.cyan('serve')}\r\n`)
      }
    })
